...
ccblocks runs a lightweight trigger every 5 minutes through the OS-native user scheduler:

- **macOS**: LaunchAgent with `StartInterval=300`
- **Linux**: systemd user timer with `OnUnitActiveSec=5min`
- **No schedule file to maintain**: pause/resume controls the scheduler
- **Simple failure handling**: success or failure, the next attempt is just the next 5-minute scheduler tick

Polling every 5 minutes keeps setup simple and avoids missing a new usage window because a fixed clock-time trigger happened while Claude was still rate-limited.
...
**Trigger mechanism:**
1. LaunchAgent/systemd timer fires every 5 minutes
